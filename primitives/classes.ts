  ...
  constructor({
    ...
    if (value !== null) {
      Object.entries(value).forEach((obj: [string, string]) => {
        ...
        Store.addStoreChildren(obj[1], `_${id}_`, this[obj[0]]);
      });
    }
  }

  static addStoreChildren(
    children: string | boolean | object | number,
    id: number | string,
    parent: Store
  ) {
    if (children instanceof Object) {
      ...
      Object.entries(children).forEach((child) => {
        parent[child[0]] = new Store({
          value: null,
          id: id,
          name: `${parent.$$__name?.replace(`['`, '')}?.['${child[0]}`,
        });

        Store.addStoreChildren(child[1], `${id}`, parent[child[0]]);
      });
    } else {
      return;
    }
