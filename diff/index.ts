...
const { types, iterableTypes, errors } = config;
...
function getKeys(x, y, type) {
  if (type === types.ARRAY) {
    const keys =
      x.length > y.length ? new Array(x.length) : new Array(y.length);
    keys.fill(0);
...
function getDiff(x, y, keepOldValInDiff = false) {
